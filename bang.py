...
def Y_d_fun(X):
    return 0.5 * (1. - torch.pow(X[:, 0:1], 2))
...
class Bangbang:
    ...
    def loss_Y(self, X_boundary, X_initial, X_interior, verbose=False):
        # 计算右边界点的个数
        N_r = torch.sum(X_boundary[:, 0]).detach(
        ...
        loss_B_R = self.criterion(
            Y_boundary[-N_r:] + dYdx_boundary[-N_r:], beta * U)
    ...
    def draw_Y(self, format='png', save=True):
        ...
        if save:
            s = time.strftime('./figures/state-%d-%H-%M-%S',
        ...
        if save:
            s = time.strftime('./figures/state_final-%d-%H-%M-%S',
                              time.localtime()) + '.' + format
            plt.savefig(s)
    ...
    def draw_U(self, format='png', save=True):
        N = 200
        temp_t = np.linspace(0., time_T, N, dtype=dtype).reshape(-1, 1)
        temp_xt = np.hstack(
        ...
        if save:
            s = time.strftime('./figures/control-%d-%H-%M-%S',
                              time.localtime()) + '.' + format
            plt.savefig(s)
