...
def Y_d_fun(X):
    x = X[:, 0:1]
    return torch.where(x < 0.5, torch.ones_like(x, device=device), torch.zeros_like(x, device=device))
...
class Burger:
    ...
    def loss_Y(self, X_boundary, X_initial, X_interior, verbose=False):
        ...
        loss_B = torch.mean(Y_boundary ** 2) * 2. + \
            self.criterion(Y_initial, Y_initial_true)
        loss = loss_F + loss_B
    ...
    def loss_P(self, X_boundary, X_final, X_interior, verbose=False):
        ...
        loss_B = torch.mean(P_boundary ** 2) * 2. + torch.mean(P_final ** 2)
        loss = loss_F + loss_B
    ...
    def load_model(self, name):
        ...
        model_P_state_dict = torch.load(
            model_path + 'model_P.pt', map_location=device)
        model_Y_state_dict = torch.load(
            model_path + 'model_Y.pt', map_location=device)
