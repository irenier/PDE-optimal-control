...
def Y_d_fun(X):
    ...
    y = X[:, 1:2]
    t = X[:, 2:3]
    return torch.pow(x, 2) * (3 - 2 * x) * torch.pow(y, 2) * (3 - 2 * y) * torch.sin(0.5 * torch.pi * t)
...
class MyGeometryXTime(geometry.GeometryXTime):
    ...
    def generate_uniform_data(self, train_n: dict):
        X_boundary = self.uniform_boundary_points(
            train_n['boundary'])

        X_x = self.geometry.uniform_points(
        ...
        X_interior = self.uniform_points(
        ...
        return X_boundary_train, torch.tensor(X_initial, device=device, requires_grad=True), torch.tensor(X_final, device=device, requires_grad=True), torch.tensor(X_interior, device=device, requires_grad=True)

    def generate_random_data(self, train_n: dict):
        X_boundary = self.random_boundary_points(
            train_n['boundary'])

        X_x = self.geometry.random_points(
            train_n['initial']).astype(dtype=dtype)
...
class Heat:
    ...
    def loss_Y(self, X_boundary, X_initial, X_interior, verbose=False):
        ...
        loss_B = torch.mean(dYdX_boundary ** 2) * 4 + \
    ...
    def loss_P(self, X_boundary, X_final, X_interior, verbose=False):
        ...
        loss_B = torch.mean(dPdX_boundary ** 2) * 4 + \
    ...
    def train(self, format='png', verbose=False):
        # 预训练
        X_boundary, X_initial, X_final, X_interior = self.geomtime.generate_uniform_data(
            self.L_train_warm_n)
        ...
        for i_epoch in range(self.iter_epoch):
            print("\nepoch %3d" % (i_epoch + 1))
            # 生成数据
            X_boundary, X_initial, X_final, X_interior = self.geomtime.generate_random_data(
                self.L_train_n)
